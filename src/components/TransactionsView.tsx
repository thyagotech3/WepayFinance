...
import { 
  ...
  ChevronDown, User, Tag, Eye, PieChart
} from 'lucide-react';
...
export const TransactionsView: React.FC<TransactionsViewProps> = ({
  ...
  const isCurrentRealMonth = useMemo(() => {
    if (viewAllPeriods) return false;
    const now = new Date();
    return selectedDate.getFullYear() === now.getFullYear() && selectedDate.getMonth() === now.getMonth();
  }, [selectedDate, viewAllPeriods]);

  // Count active transactions in other periods
  const otherMonthsTxCount = useMemo(() => {
    const active = transactions.filter((t) => t.status !== 'reverted' && t.status !== 'deleted');
    const thisMonth = active.filter((t) => t.date && t.date.startsWith(selectedMonthKey));
    return active.length - thisMonth.length;
  }, [transactions, selectedMonthKey]);

  // Filter transactions by selected month or all
  const monthTransactions = useMemo(() => {
    if (viewAllPeriods) {
      return [...transactions].sort((a, b) => new Date(b.date || 0).getTime() - new Date(a.date || 0).getTime());
    }
    ...
    return monthTx;
  }, [transactions, selectedMonthKey, viewAllPeriods]);
  ...
  return (
    <div className="space-y-2.5 sm:space-y-4 max-w-xl lg:max-w-7xl mx-auto pb-24">
      ...
      <div className="bg-[#0e1224] border border-slate-800/80 rounded-2xl p-2 sm:p-3 shadow-lg flex items-center justify-between gap-2">
        <button
          type="button"
          ...
          <div className="flex flex-col text-left min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-wider leading-tight">
                {viewAllPeriods ? 'Visão Geral' : 'Histórico de lançamento'}
              </span>
              <button
                type="button"
                onClick={() => setViewAllPeriods(!viewAllPeriods)}
                className={`text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded-md font-bold transition-all border cursor-pointer ${
                  viewAllPeriods
                ...
                title={viewAllPeriods ? 'Filtrar por mês selecionado' : 'Ver todos os lançamentos'}
              >
                {viewAllPeriods ? 'Filtrar por Mês' : 'Ver Todos'}
              </button>
            </div>
            <div className="flex items-center gap-1.5 truncate">
              ...
              {isCurrentRealMonth && (
                <span className="hidden xs:inline px-1.5 py-0.2 rounded-full bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-[9px] font-extrabold uppercase tracking-wider shrink-0">
                  Atual
                </span>
              )}
            </div>
      ...
      {/* Notice when transactions exist in other months */}
      {!viewAllPeriods && otherMonthsTxCount > 0 && (
        <div className="bg-purple-950/30 border border-purple-500/30 rounded-xl px-3 py-2 flex items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 text-slate-300 truncate">
            <span className="text-purple-400 font-bold shrink-0">📌 Dica:</span>
            <span className="text-[11px] truncate">
              Você tem <strong>{otherMonthsTxCount}</strong> {otherMonthsTxCount === 1 ? 'lançamento' : 'lançamentos'} em outros períodos.
            </span>
          </div>
          <button
            type="button"
            onClick={() => setViewAllPeriods(true)}
            className="px-2 py-1 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-[10px] font-bold shrink-0 transition-colors cursor-pointer"
          >
            Ver Todos
          </button>
        </div>
      ...
      <div className="bg-[#0e1224] border border-slate-800/80 rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-800/60">
          {displayedTransactions.length === 0 ? (
            <div className="text-center py-14 px-4 text-slate-500 text-xs space-y-3">
              ...
              <h4 className="text-sm font-bold text-slate-300">
                {viewAllPeriods ? 'Nenhum lançamento encontrado' : `Nenhum lançamento em ${formattedMonthYear}`}
              </h4>
              <p className="text-[11px] text-slate-400 max-w-xs mx-auto">
                {hasActiveFilters
                  ? 'Tente ajustar os filtros, pesquisar por outro termo ou limpar a seleção.'
                  : !viewAllPeriods && otherMonthsTxCount > 0
                  ? `Existem ${otherMonthsTxCount} lançamentos registrados em outros períodos ou anos.`
              ...
              <div className="flex items-center justify-center gap-2 flex-wrap pt-1">
                ...
                {!viewAllPeriods && otherMonthsTxCount > 0 && (
                  <button
                    type="button"
                    onClick={() => setViewAllPeriods(true)}
                    className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-purple-500/40 text-purple-300 text-xs font-bold transition-all cursor-pointer shadow-md flex items-center gap-1.5"
                  >
                    <Calendar className="w-3.5 h-3.5" />
                    <span>Ver {otherMonthsTxCount} lançamentos de outros períodos</span>
                  </button>
                )}
              </div>
            </div>
